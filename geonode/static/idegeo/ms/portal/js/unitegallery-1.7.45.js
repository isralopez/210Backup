...
function UGFunctions() {
    ...
    /** move to full screen mode fullscreen ID - the ID of current fullscreen **/
    ...
    /** exit full screen mode return if operation success (or if fullscreen mode supported) **/
    ...
    this.addFullScreenChangeEvent = function(func) {
        if (document["webkitCancelFullScreen"]) addEvent("webkitfullscreenchange", document, func);
        else if (document["msExitFullscreen"]) addEvent("MSFullscreenChange", document, func);
        else if (document["mozCancelFullScreen"]) addEvent("mozfullscreenchange", document, func);
        else addEvent("fullscreenchange", document, func);
    }
    ...
    /** get image inside parent data by image (find parent and size) scaleMode: fit, down, fill, fitvert **/
    ...
    /** get data of image inside parent scaleMode: fit, down, fill, fitvert **/
    this.getImageInsideParentData = function(objParent, originalWidth, originalHeight, scaleMode, objPadding, maxWidth, maxHeight) {
        ...
        if (imageTop == 0 || imageHeight == "100%") objOutput.imageBottom = null;
        else objOutput.imageBottom = imageTop + imageHeight;
        objOutput.style = style;
        return (objOutput);
    }
    /** get element center position inside parent even if the object bigger than the parent **/
    ...
    /** get the center of the element includeParent - including left / right related to the parent **/
    ...
    /** get mouse position from the event optimised to every device **/
    ...
    /** get image oritinal size if originalWidth, originalHeight is set, just return them. **/
    ...
    /** tells if the mouse point inside image the mouse point is related to image pos **/
    ...
    /** get element relative position according the parent if the left / top is offset text (left, center, right / top, middle, bottom) the element can be number size as well **/
    ...
    /** zoom image inside parent the mouse point is page offset position, can be null return true if zoomed and false if not zoomed **/
    this.zoomImageInsideParent = function(objImage, zoomIn, step, point, scaleMode, maxZoomRatio, objPadding) {
        ...
        // set position: if zoom out and mouse point not inside image, get the image to center
    ...
    /** place some element to some position if the left / top is offset text (left, center, right / top, middle, bottom) then put it in parent by the offset. **/
    ...
    /** place element inside parent center. the element should be absolute position **/
    ...
    /** PLACE IMAGE INSIDE PARENT, SCALE IT BY THE OPTIONS AND SCALE IT TO FIT THE PARENT. SCALEMODE: FIT, DOWN, FILL **/
    this.placeImageInsideParent = function(urlImage, objParent, originalWidth, originalHeight, scaleMode, objPadding) {
        ...
        /***************************************************************************
            ADD-MOD
        ***************************************************************************/
        var itemTPEcss = 'display:none;position:absolute;top:0px;left:0px;width:100%;height:100%;background-color:rgba(0,0,0,0.00);zIndex:2;cursor:pointer;';
        htmlImage += '<div class="ug-button-image" style="' + itemTPEcss + '"></div>';
        /**************************************************************************/
    ...
    /** scale image to fit parent, and place it into parent parent can be width , height, or object **/
    ...
    /** resize image to fit the parent, scale it by the options scaleMode: fit, down, fill **/
    ...
    /** convert custom prefix options prefix - the prefix. base - after the prefix text. example lightbox_slider_option (lightbox - prefix, slider - base) **/
    ...
    /** get number of items in space width gap even item sizes by lowest **/
    this.getNumItemsInSpace = function(spaceSize, itemsSize, gapSize) {
        var numItems = Math.floor((spaceSize + gapSize) / (itemsSize + gapSize));
        return (numItems);
    }
    /** get number of items in space width gap even item sizes by Math.round **/
    ...
    this.handleScrollTop = function(storedEventID) {
        ...
        if (objData.scrollDir === null) {
            if (Math.abs(objData.diffMouseX) > horPass) objData.scrollDir = "hor";
            else
    ...
    /** check if current jquery version is more then minimal version version can be "1.8.0" for example **/
    ...
    /** add to button ug-nohover class on ipad need to be processed in css **/
    ...
    /** function checks if enought time passsed between function calls. good for filtering same time events **/
    this.isTimePassed = function(handlerName, timeLimit) {
        if (!timeLimit) var timeLimit = 100;
        var currentTime = jQuery.now();
        if (g_temp.timeCache.hasOwnProperty(handlerName) == false) lastTime = 0;
        else lastTime = g_temp.timeCache[handlerName];
    ...
    /** validate click and touchstart events. if click comes after touchstart, return false. **/
...
function UGThumbsGeneral() {
    ...
    /** set thumb size with all the inside components (without the image). else, set to all the thumbs **/
    ...
    function setThumbBorderEffect(objThumb, borderWidth, borderColor, noAnimation) {
        ...
        /* if(g_options.thumb_border_width == g_options.thumb_over_border_width && g_options.thumb_border_width == g_options.thumb_selected_border_width)
        ...
        if (noAnimation && noAnimation === true) {
            ...
            if (borderWidth == 0) objBorderOverlay.hide();
            else objBorderOverlay.show();
        } else {
            if (borderWidth == 0) objBorderOverlay.stop().fadeOut(g_options.thumb_transition_duration);
            else objBorderOverlay.show().stop().fadeIn(g_options.thumb_transition_duration);
    ...
    function redrawThumbStyle(objThumb) {
        if (isThumbSelected(objThumb) == true) setThumbSelectedStyle(objThumb, true, "redraw");
        else t.setThumbNormalStyle(objThumb, true, "redraw");
    }
    /** place thumb image retrun - false, if already loaded return true - if was set on this function isForce - set the image anyway **/
    ...
    /** set custom thumbs allowedEffects - border, overlay, image **/
...
/** thumbs class addon to strip gallery **/
function UGThumbsStrip() {
    ...
    /** store strip size and strip active size in vars do after all strip size change **/
    function storeStripSize(size) {
        g_sizes.stripSize = size;
        if (g_isVertical == false) g_sizes.stripActiveSize = g_sizes.stripSize - g_options.strip_padding_left - g_options.strip_padding_right;
        else g_sizes.stripActiveSize = g_sizes.stripSize - g_options.strip_padding_top - g_options.strip_padding_bottom;
    ...
    function setInnerStripSize(innerSize) {
        if (g_isVertical == false) g_objStripInner.width(innerSize);
        else g_objStripInner.height(innerSize);
    ...
    function positionThumbs() {
        ...
        for (i = 0; i < arrThumbs.length; i++) {
            ...
            if (g_isVertical == false) posx += objThumb.outerWidth() + g_options.strip_space_between_thumbs;
    ...
    function scrollToThumb(objThumb) {
        ...
        if (objThumbPos.min < objBounds.minPosThumbs) {
            ...
            if (prevThumb.length) scrollToThumbMin(prevThumb);
            else scrollToThumbMin(objThumb);
        } else if (objThumbPos.max > objBounds.maxPosThumbs) {
            var nextThumb = objThumb.next();
            if (nextThumb.length) scrollToThumbMax(nextThumb);
            else scrollToThumbMax(objThumb);
        }
    }
    ...
    function alignInnerStrip() {
        ...
        if (g_isVertical == false) g_functions.placeElement(g_objStripInner, g_options.strip_thumbs_align, 0);
        else g_functions.placeElement(g_objStripInner, 0, g_options.strip_thumbs_align);
    }
    ...
    /** get bounds, if the thumb not in them, it need to be scrolled minPosThumbs, maxPosThumbs - the min and max position that the thumbs should be located to be visible **/
    ...
    this.getInnerStripPos = function() {
        ...
        else return g_objStripInner.position().top;
    }
    /** get inner strip limits **/
    this.getInnerStripLimits = function() {
        var output = {};
        if (g_isVertical == false) output.maxPos = g_options.strip_padding_left;
        else output.maxPos = g_options.strip_padding_top;
    ...
    /** set size of the strip the height size is set automatically from options **/
...
/** touch thumbs control class addon to strip gallery **/
function UGTouchThumbsControl() {
    ...
    function getMouseOrientPosition(event) {
        ...
        else return (g_functions.getMousePosition(event).pageY);
    }
    ...
    /** on touch start - start the motion @param event **/
...
function UGPanelsBase() {
    ...
    this.setHtml = function(g_objPanelArg) {
        ...
        if (g_temp.panelType == "strip") var enable_handle = g_options.strippanel_enable_handle;
        else var enable_handle = g_options.gridpanel_enable_handle;
        // add handle
    ...
    /** place the panel without animation @param panelDest **/
    ...
    this.openPanel = function(noAnimation) {
        ...
        if (noAnimation === false) placePanelAnimation(g_temp.originalPos, onPanelSlideFinish);
        else {
    ...
    this.closePanel = function(noAnimation) {
        ...
        if (noAnimation === false) placePanelAnimation(panelDest, onPanelSlideFinish);
        else {
    ...
    /** set the panel disabled at start, called after init before setHtml it's enabled again after timeout end **/
...
function UGPanelHandle() {
    ...
    function onHandleClick(event) {
        ...
        if (g_panel.isPanelClosed()) g_panel.openPanel();
        else g_panel.closePanel();
    }
    /** init events **/
    this.initEvents = function() {
...
function UGStripPanel() {
    ...
    function setElementsSize() {
        if (g_options.strippanel_vertical_type == false) setElementsSize_hor();
        else setElementsSize_vert();
    }
    ...
    function placeElements() {
        if (g_options.strippanel_vertical_type == false) placeElements_hor();
        else placeElements_vert();
    ...
    function onNextButtonClick(objButton) {
        ...
        if (g_options.strippanel_buttons_role == "advance_item") g_gallery.nextItem();
        else g_objStrip.scrollForeward();
    }
    /** on previous button click **/
    function onPrevButtonClick(objButton) {
        if (g_functions.isButtonDisabled(objButton)) return (true);
        if (g_options.strippanel_buttons_role == "advance_item") g_gallery.prevItem();
        else g_objStrip.scrollBack();
    }
    ...
    function checkSideButtons() {
        ...
        else g_functions.enableButton(g_objButtonNext);
    }
    /** on strip move event **/
    function onStripMove() {
        checkSideButtons();
    }
    /** on item change event, disable or enable buttons according the images position **/
    function onItemChange() {
        if (g_gallery.isLastItem()) g_functions.disableButton(g_objButtonNext);
        else g_functions.enableButton(g_objButtonNext);
        if (g_gallery.isFirstItem()) g_functions.disableButton(g_objButtonPrev);
...
function UGGridPanel() {
    ...
    function setPanelSize() {
        ...
        // set panel size
        if (g_temp.isHorType == true) g_temp.panelHeight = gridSize.height + g_options.gridpanel_padding_border_top + g_options.gridpanel_padding_border_bottom;
        else g_temp.panelWidth = gridSize.width + g_options.gridpanel_padding_border_left + g_options.gridpanel_padding_border_right;
    ...
    function placeElementsVert() {
        if (g_temp.arrowsVisible == true) placeElementsVert_arrows();
        else placeElements_noarrows();
    }
    ...
    function placeElementsHor() {
        if (g_temp.arrowsVisible == true) placeElementsHor_arrows();
        else placeElements_noarrows();
    }
    ...
    function placeElementsHorType() {
        if (g_temp.arrowsVisible == true) placeElementsHorType_arrows();
        else placeElementHorType_noarrows();
    }
    /** place the arrows **/
    function placeElements() {
        if (g_temp.isHorType == false) {
            if (g_options.gridpanel_vertical_scroll == true) placeElementsVert();
            else placeElementsHor();
        } else {
...
function UGThumbsGrid() {
    ...
    function setHtml(parentContainer) {
        ...
        // put the thumbs to inner strip
        if (g_temp.isTilesMode == true) g_tilesDesign.setHtml(g_objInner);
        else g_thumbs.setHtmlThumbs(g_objInner);
    }
    ...
    function getThumbsSize() {
        if (g_temp.isTilesMode == true) var objThumbSize = g_tilesDesign.getGlobalTileSize();
        else var objThumbSize = g_thumbs.getGlobalThumbSize();
        return (objThumbSize);
    }
    ...
    /** init grid size **/
    function initSizeParams() {
        if (g_temp.isHorizontal == false) initSizeParamsVert();
        else initSizeParamsHor();
    }
    /** goto pane by index **/
    ...
    function positionThumbs() {
        if (g_temp.isHorizontal == false) { // position vertical type
            if (g_temp.isNavigationVertical) positionThumb_vert();
            else positionThumb_hor();
        } else {
    ...
    function getInnerSize() {
        ...
        else return (g_temp.innerWidth);
    }
    /** get pane width or height according the orientation **/
    function getPaneSize() {
        if (g_temp.isNavigationVertical == true) return (g_temp.gridHeight);
        else return (g_temp.gridWidth);
    }
    /** get object of iner position move **/
    function getInnerPosObj(pos) {
        var obj = {};
        if (g_temp.isNavigationVertical == true) obj.top = pos + "px";
        else obj.left = pos + "px";
        return (obj);
    }
    ...
    function onTouchEnd(event) {
        ...
        var direction = (diff > 0) ? "prev" : "next";
        if (direction == "next") {
            if (g_options.grid_carousel == false && t.isLastPane()) animateToCurrentPane();
            else t.nextPane();
        } else {
    ...
    /** set max width, the width will be corrected by the number of items set vertical type **/
    ...
    /** set max height, the height will be corrected by the number of items set the vertical type **/
    this.setMaxHeight = function(maxHeight) {
        g_temp.gridHeight = maxHeight;
        g_temp.isMaxHeight = true;
        g_temp.isHorizontal = false;
    }
    /** goto some pane force skip current pane checks **/
    ...
    this.attachNextPaneButton = function(objButton) {
        ...
        g_objThis.on(t.events.PANE_CHANGE, function() {
            if (t.isLastPane()) objButton.addClass("ug-button-disabled");
            else objButton.removeClass("ug-button-disabled");
        });
    }
...
function UGTiles() {
    ...
    function setHtml(objParent) {
        if (!objParent) {
            if (g_objParent) objParent = g_objParent;
            else var objParent = g_objWrapper;
        }
        g_objParent = objParent;
    ...
    function fillTilesVars() {
        ...
        if (g_options.tiles_exact_width == true) {
            ...
            if (numCols < g_vars.minCols) numCols = g_vars.minCols;
            else
    ...
    function runColumnsType() {
        ...
        g_functions.checkImagesLoaded(objImages, function() {
            ...
            if (g_temp.isFirstPlaced == true) g_gallery.triggerEvent(t.events.TILES_FIRST_PLACED); // set to false
    ...
    /** redistribute heights in column returns current item sizes **/
...
function UGTileDesign() {
    ...
    function setHtmlThumb(objThumbWrapper, objItem) {
        ...
        if (g_options.tile_enable_icons) {
            ...
            else var objOverlay = objThumbWrapper.children(".ug-thumb-overlay");
            objOverlay.append(htmlAdd);
            var objButtonZoom = objOverlay.children("." + iconPlayClass);
            if (objButtonZoom.length == 0) objButtonZoom = null;
            else objButtonZoom.hide();
            var objButtonLink = objOverlay.children(".ug-icon-link");
            if (objButtonLink.length == 0) objButtonLink = null;
            else objButtonLink.hide();
    ...
    function getTextPanelHeight(objTile) {
        if (g_temp.isTextpanelOutside == true) var objTextPanel = getTextPanelCloneElement(objTile);
        else var objTextPanel = getTextPanelElement(objTile);
    ...
    /** position tile images elements width, height - tile width height **/
    ...
    /** position text panel panelType - default or clone **/
    ...
    /** on tile click on mobile devices on normal state set the tile over state **/
    ...
    /** resize tile. If no size given, resize to original size the resize mode taken from resize modes constants, default is full **/
    ...
    /** trigger size change events the force is only for fixed size mode **/
    this.triggerSizeChangeEvent = function(objTile, isForce) {
        if (!objTile) return (false);
        if (!isForce) var isForce = false;
        g_objWrapper.trigger(g_temp.eventSizeChange, [objTile, isForce]);
    }
    /** trigger size change event to all tiles the force is only for fixed mode **/
...
/** avia control class addon to strip gallery **/
function UGAviaControl() {
    ...
    /** get inner position according the orientation taken by the mouse position **/
    function getInnerPos(mousePos) {
        if (g_isVertical == false) return getInnerPosX(mousePos);
        else return getInnerPosY(mousePos);
    }
    /** move the strip to mouse position on it mousex - mouse position relative to the document **/
...
/** slider class addon to strip gallery **/
function UGSlider() {
    var t = this,
        g_objThis = jQuery(t);
    var g_gallery = new UniteGalleryMain(),
    ...
    /** show the preloader show the index, so only the current index load will hide. **/
    ...
    function setImageToSlide(objSlide, objItem, isForce) {
        ...
        if (currentImage == urlImage && isForce !== true) {
            ...
            if (objItem.isBigImageLoaded == true) {
                ...
                if (slideType != "image" && g_options.slider_video_constantsize == true) var objImageData = scaleImageConstantSize(objImage, objItem);
                else var objImageData = g_functions.getImageInsideParentData(objItemWrapper, objItem.imageWidth, objItem.imageHeight, scaleMode, objPadding);
                ...
                objImage.on("load", function() {
                    ...
                    var objImageData = {};
                    if (slideType != "image" && g_options.slider_video_constantsize == true) scaleImageConstantSize(objImage, objItem);
                    else {
                        objImageData = g_functions.scaleImageFitParent(objImage, objItem.imageWidth, objItem.imageHeight, scaleMode, objPadding);
                    }
                    objImage.fadeTo(0, 1);
    ...
    function setItemToSlide(objSlide, objItem) {
        try {
            ...
            if (g_options.slider_enable_links == true && objItem.type == "image") {
                if (objItem.link) objSlide.addClass("ug-slide-clickable");
                else objSlide.removeClass("ug-slide-clickable");
            }
    ...
    function animateOpacity(objItem, opacity, completeFunction) {
        if (completeFunction) objItem.fadeTo(g_options.slider_transition_speed, opacity, completeFunction);
        else objItem.fadeTo(g_options.slider_transition_speed, opacity);
    }
    /** do fade transition **/
    ...
    function hideControls(noAnimation) {
        ...
        if (noAnimation === true) objControls.stop().fadeTo(0, 0).hide();
        else {
    ...
    /** show controls only if they meaned to be shown @param noAnimation **/
    ...
    function showControls(noAnimation) {
        ...
        if (!noAnimation) var noAnimation = false;
        var objControls = getControlsObjects();
        if (noAnimation === true) objControls.stop().show();
        else {
    ...
    function toggleControls() {
        if (g_temp.isControlsVisible == false) showControls();
        else hideControls();
    }
    /** set controls mode modes: image, video **/
    ...
    function onItemChange(data, arg_objItem, role) {
        ...
        ***************************************************************************/
        ...
        var itemTPE = objItem.dataTPE;
        var itemLNK = objItem.dataLNK;
        var itemTRG = objItem.dataTRG;
        var itemMPE = objItem.dataMPE;
        jQuery(currentSlide).siblings().removeClass("ug-slide-image");
        jQuery(currentSlide).siblings().removeClass("ug-slide-video");
        jQuery(currentSlide).siblings().removeClass("ug-item-typed");
        jQuery(currentSlide).siblings().find(".ug-button-image").css("display", "none");
        /**************************************************************************/
        if (objItem.type == "image") {
            ...
            /***************************************************************************
                ADD-MOD
            ***************************************************************************/
            jQuery(currentSlide).addClass("ug-slide-image");
            if (itemTPE != "image") {
                jQuery(currentSlide).addClass("ug-item-typed");
                jQuery(currentSlide).find(".ug-button-image").css("display", "block");
            }
            /**************************************************************************/
        } else {
            ...
            /***************************************************************************
                ADD-MOD
            ***************************************************************************/
            jQuery(currentSlide).addClass("ug-slide-video");
            /**************************************************************************/
        }
        /***************************************************************************
            ADD-MOD [ *** TEMPORAL-DEMO *** ]
        ***************************************************************************/
        if (itemLNK === null) {
            // UG-TYPE = IMAGE or VIDEO
            itemLNK = (objItem.type).toUpperCase();
        }
        var codeTITLE = '<div class="ug-button-image-title">' + itemTPE.toUpperCase() + ' | ' + itemLNK + '</div>'
        // jQuery(currentSlide).find(".ug-button-image").append(codeTITLE);
        jQuery(currentSlide).find(".ug-button-image").after(codeTITLE);
        jQuery(currentSlide).find(".ug-button-image-title").css({
            "display": "block",
    ...
    function onActualClick() {
        ...
        ***************************************************************************/
        ...
        var itemTRG = objItem.dataTRG;
        var itemMPE = objItem.dataMPE;
        // trace("TPE: " + itemTPE + " LNK: " + itemLNK + " TRG: " + itemTRG + " MPE: " + itemMPE);
        /*------------------------------------------------------------------------*/
        if ((itemTPE != "video") && (itemTPE !== null)) {
            if (itemLNK !== null) {
                if (itemTPE == "link") {
                    if (itemTRG === null) {
                        // LINK > PARENT
                        window.location.href = itemLNK;
                    } else {
                        // LINK > BLANK
                        window.open(itemLNK);
                    }
                } else if (itemTPE == "modal") {
                    if ((itemMPE != "") && (itemMPE !== null)) {
                        var modalCONT = '<div class="modal-wrapper">' + itemLNK + '</div>';;
                        var modalTHEM = 'light';
                        var modalCLAS = 'large';
                        // MODAL-TYPE = image OR iframe OR url
                        if (itemMPE == "url") {
                            modalCONT = 'url:' + itemLNK;
                            modalTHEM = 'light';
                            modalCLAS = 'xlarge';
                        } else if (itemMPE == "iframe") {
                            modalCONT = '<div class="modal-wrapper"><iframe src="' + itemLNK + '" width="" height="" sandbox></iframe></div>';
                            modalTHEM = 'light';
                            modalCLAS = 'large';
                        } else if (itemMPE == "image") {
                            modalCONT = '<div class="modal-wrapper"><img src="' + itemLNK + '"></div>';
                            modalTHEM = 'supervan';
                            modalCLAS = 'xlarge';
                        }
                ...
                } else if (itemTPE == "file") {
                    // LINK > BLANK
                    window.open(itemLNK);
                } else {
                    // ITEM-TYPE = NULL
                }
            } else if (itemTPE == "image") {
                // ITEM-LINK = NULL
            } else {
                // NPN
            }
    ...
    this.________EXTERNAL_GENERAL___________ = function() {};
    /** init function for avia controls options: width / height **/
    ...
    /** is mouse inside slide image get mouse position from event **/
    ...
    this.setItem = function(objItem, forseTransition, role) {
        ...
        if (isFirstSlide) {
            ...
            var numItems = g_gallery.getNumItems();
            if (role == "next") direction = "left";
            else if (role == "prev" || currentIndex > nextIndex) direction = "right";
            else if (currentIndex > nextIndex) direction = "right";
    ...
    this.stopSlideAction = function(objSlide, isPause) {
        ...
        if (isPause === true) g_objVideoPlayer.pause();
        else g_objVideoPlayer.hide();
        // trace("stop action");
    }
    ...
    this.getScaleMode = function(objSlide) {
        ...
        else return (g_options.slider_scale_mode);
    }
    /** get slider objects **/
    this.getObjects = function() {
        var obj = {
    ...
    /** set the slider size works well on resize too. **/
...
function UGTextPanel() {
    ...
    function setCss() {
        ...
        if (g_options.textpanel_enable_title == true) {
            ...
            if (g_options.textpanel_title_bold !== null) {
                if (g_options.textpanel_title_bold === true) objCssTitle["font-weight"] = "bold";
                else objCssTitle["font-weight"] = "normal";
            }
            // set additional css
        ...
        if (g_options.textpanel_enable_description == true) {
            ...
            if (g_options.textpanel_desc_bold !== null) {
                if (g_options.textpanel_desc_bold === true) objCssDesc["font-weight"] = "bold";
                else objCssDesc["font-weight"] = "normal";
            }
            // set additional css
    ...
    this.refresh = function(toShow, noPosition, panelWidth, panelHeight) {
        setCss();
        if (!panelWidth) t.setSizeByParent();
        else t.setPanelSize(panelWidth, panelHeight);
        t.positionElements(false);
...
function UGBullets() {
    ...
    var g_options = {
        bullets_skin: "", // skin of the bullets, if empty inherit from gallery skin
    ...
    this.init = function(gallery, customOptions, numBullets) {
        g_gallery = gallery;
        if (numBullets) g_numBullets = numBullets;
        else g_numBullets = g_gallery.getNumItems();
    ...
    function setHtmlBullets() {
        ...
        for (var i = 0; i < g_numBullets; i++) {
            if (i == 0) html += "<div class='ug-bullet'></div>";
            else html += "<div class='ug-bullet'" + addHtml + "></div>";
        }
    ...
    /** init the bullets events trigger bullet click event **/
    function initEvents() {
    ...
    this.isActive = function(index) {
        validateIndex(index);
        if (typeof index != "number") var objBullet = index;
        else {
...
function UGTouchSliderControl() {
    ...
    /** check tab event occured invokes on touchend event on the slider object **/
    ...
    function handleSliderDrag(event) {
        ...
        if (diff > 0 && currentPosx > 0) currentPosx = currentPosx / 3;
        else if (diff < 0) {
...
function UGZoomSliderControl() {
    ...
    /** get fit image to slider scale mode the fill become fit **/
...
function UGSoundCloudAPI() {
    ...
    function putSoundActually(divID, trackID, width, height, isAutoplay) {
        ...
        if (isAutoplay === true) url += "&amp;auto_play=true";
        else url += "&amp;auto_play=false";
...
function UGHtml5MediaAPI() {
    ...
    this.loadAPI = function() {
        ...
        ***************************************************************************/
        ...
        g_ugFunctions.loadJs("assets/cdnjs/mediaelement-2.18.1.min.js", false);
        g_ugFunctions.loadCss("assets/cdnjs/mediaelementplayer-2.18.1.min.css", false);
        /**************************************************************************/
        g_ugHtml5MediaAPI.isAPILoaded = true;
    }
...
function UGYoutubeAPI() {
    ...
    /** on player state change event trigger events **/
    ...
    this.changeVideo = function(videoID, isAutoplay) {
        if (t.isPlayerReady() == false) return (false);
        if (isAutoplay && isAutoplay == true) g_player.loadVideoById(videoID, 0, "large");
        else g_player.cueVideoById(videoID, 0, "large");
    }
    /** get if the player is ready **/
    this.isPlayerReady = function() {
        if (g_isPlayerReady && g_player) return (true);
...
function UGVideoPlayer() {
    ...
    this.playYoutube = function(videoID, isAutoplay) {
        ...
        if (g_youtubeAPI.isPlayerReady() == true && g_temp.standAloneMode == true) g_youtubeAPI.changeVideo(videoID, isAutoplay);
        else {
...
function UniteGalleryMain() {
    ...
    function fillItemByChild(objChild) {
        ...
        if (tagname == "img") {
            ...
            /***************************************************************************
                ADD-MOD [GET-ITEM-DATA]
            ***************************************************************************/
            var dataTPE = objChild.data("xtype");
            var dataLNK = objChild.data("link");
            var dataTRG = objChild.data("target");
            var dataMPE = objChild.data("mtype");
            if (typeof(dataTPE) == "undefined") dataTPE = null;
            if (typeof(dataLNK) == "undefined") dataLNK = null;
            if (typeof(dataTRG) == "undefined") dataTRG = null;
            if (typeof(dataMPE) == "undefined") dataMPE = null;
            objItem.dataTPE = dataTPE;
            objItem.dataLNK = dataLNK;
            objItem.dataTRG = dataTRG;
            objItem.dataMPE = dataMPE;
            /**************************************************************************/
    ...
    /** on item big image loaded event function. Update image size and that the image has been preloaded can be called from another objects like the slider **/
    ...
    /** check that all items started preloading, if do, set flag g_temp.isAllItemsPreloaded to true **/
    ...
    this.getNextItem = function(index, forceCarousel) {
        ...
        if (nextIndex >= g_numItems) {
            if (g_options.gallery_carousel == true || forceCarousel === true) nextIndex = 0;
            else return (null);
        }
    ...
    this.getPrevItem = function(index) {
        ...
        if (prevIndex < 0) {
            if (g_options.gallery_carousel == true || forceCarousel === true) prevIndex = g_numItems - 1;
            else return (null);
        }
    ...
    /** set next button element set onclick event **/
    this.setNextButton = function(objButton) {
        // register button as a unite gallery belong
        objButton.data("ug-button", true);
        g_functions.setButtonOnClick(objButton, t.nextItem);
    }
    /** set prev button element set onclick event **/
    this.setPrevButton = function(objButton) {
        // register button as a unite gallery belong
        objButton.data("ug-button", true);
        g_functions.setButtonOnClick(objButton, t.prevItem);
    }
    /** set fullscreen button to enter / exit fullscreen. on fullscreen mode ug-fullscreenmode class wil be added **/
    ...
    /** cache items, put to cache array by id the items must be unprocessed yet **/
    ...
    /** retrigger event from another objects the second parameter will be the second object **/
    ...
    this.togglePlayMode = function() {
        if (t.isPlayMode() == false) t.startPlayMode();
        else t.stopPlayMode();
    }
    /** unselect all items **/
    ...
    /** select some item the input can be index or object role - the role of the object who selected the item **/
    ...
    /** exit full screen **/
    this.exitFullScreen = function() {
        if (g_temp.isFakeFullscreen == true) exitFakeFullscreen();
        else g_functions.exitFullscreen();
    }
    /** toggle fullscreen **/
    ...
    /** resize the gallery noevent - initally false **/
    ...
    /** set size class to the wrapper this can work to any other wrapper too **/
    ...
    this.triggerEvent = function(event, arrParams) {
        if (!arrParams) g_objGallery.trigger(event);
        else {
    ...
    /** request new items isForce - don't take from cache **/
...
function UGLightbox() {
    ...
    function putLightboxHtml() {
        ...
        if (g_objTextPanel) {
            if (g_objTopPanel) g_objTextPanel.appendHTML(g_objTopPanel);
            else g_objTextPanel.appendHTML(g_objWrapper);
        }
    }
    ...
    /** position text panel for wide size - wrapper size **/
    ...
    /** handle slider image height according the textpanel height refresh the slider if the height is not in place **/
    ...
    /** handle text panel width on compact mode, run when the image is ready. Set top position of the panel as well position numbers as well **/
    ...
    function positionElements() {
        ...
        if (g_objTextPanel) {
            ...
            if (g_temp.isCompact == false) positionTextPanelWide(size);
            else {
                showTextPanel();
                showNumbers();
            }
    ...
    /** on zoom change move the assets of compact to their places **/
    function onZoomChange() {
        if (g_temp.isCompact == false) return (true);
    ...
    /** after return slider to it's place show close button **/
    ...
    /** after put image to the slide position compact elements **/
    ...
    /** on item change update numbers text and text panel text/position **/
    ...
    /** on mouse move event show arrows if inside image **/
    ...
    this.close = function(fromWhere) {
        ...
        var slideType = g_objSlider.getSlideType();
        if (slideType != "image") g_objWrapper.hide();
        else {
...
function UGCarousel() {
    ...
    function getTileClones(objTile, numClones, dir) {
        ...
        for (var i = 0; i < numClones; i++) {
            if (dir == "prev") var itemToAdd = g_gallery.getPrevItem(index, true);
            else var itemToAdd = g_gallery.getNextItem(index, true);
            if (!itemToAdd) {
    ...
    function addTiles(numTiles, dir) {
        if (dir == "left") var anchorTile = getFirstTile();
        else var anchorTile = getLastTile();
        var clonesType = (dir == "left") ? "prev" : "next";
        var arrNewTiles = getTileClones(anchorTile, numTiles, clonesType);
        jQuery.each(arrNewTiles, function(index, objTile) {
            if (dir == "left") g_objInner.prepend(objTile);
            else g_objInner.append(objTile);
    ...
    function removeTiles(numTilesToRemove, direction) {
        ...
        for (var i = 0; i < numTilesToRemove; i++) {
            if (direction == "left") jQuery(arrTiles[i]).remove();
            else {
    ...
    this.setPlayPauseButton = function(objButton) {
        ...
        g_functions.setButtonOnClick(objButton, function() {
            if (g_temp.isPlayMode == false || g_temp.isPaused == true) t.startAutoplay();
            else t.stopAutoplay();
        });
    }
    /** return if passed some significant movement **/
...
function UGTabs() {
    ...
    function initTabs(gallery, customOptions) {
        ...
        g_objGallery = jQuery(g_gallery);
        g_options = jQuery.extend(g_options, customOptions);
        if (g_options.tabs_type == "select") g_objSelect = jQuery(g_options.tabs_container);
        else g_objTabs = jQuery(g_options.tabs_container + " .ug-tab");
    }
    /** run the tabs **/
    function runTabs() {
        initEvents();
    ...
    /** init tabs events **/
    function initEvents() {
        if (g_options.tabs_type == "select") g_objSelect.change(onSelectChange);
        else g_objTabs.click(onTabClick);
    }
    /** destroy **/
    this.destroy = function() {
...
function UG_API(gallery) {
    ...
    this.resize = function(width, height) {
        if (height) g_gallery.resize(width, height);
        else g_gallery.resize(width);
    }
